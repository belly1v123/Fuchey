        ...
        switch (event_id) {
            ...
            case WIFI_EVENT_STA_DISCONNECTED: {
                ...
                if (!self->has_credentials()) {
                    ESP_LOGW(TAG, "WiFi disconnected — no credentials saved, giving up");
                    Events::Event fail_evt{};
                    fail_evt.type = Events::EventType::WIFI_CONNECT_FAILED;
                    if (Events::g_ui_queue) Events::post(Events::g_ui_queue, fail_evt);
                    break;
                }

                if (reason == WIFI_REASON_NO_AP_FOUND) {
                    self->m_reconnect_attempts++;
                    if (self->m_reconnect_attempts >= self->MAX_RECONNECT_ATTEMPTS) {
                        ESP_LOGE(TAG, "WiFi network not found after %u attempts — giving up",
                ...
                ESP_LOGW(TAG, "WiFi disconnected (reason %u) — reconnecting... attempt %u",
                         reason, self->m_reconnect_attempts);
