...
// Proves SpritePlayer + draw_sprite + flush path on hardware.
// Serial: `anim` to enter, BACK to leave. Swap kTestAnim for any
// SpriteAnim generated by scripts/convert_sprite.py (e.g. Yeti 96x96x12).
...
void UIManager::render_anim_test() {
    ...
    uint32_t now = static_cast<uint32_t>(esp_timer_get_time() / 1000);
    if (!s_started) {
        s_player.set_anim(&kTestAnim, now);
        s_started = true;
    }
    s_player.tick(now);

    m_display.draw_text_centered(8, "ANIM TEST", Display::FontSize::MEDIUM, TFT_CYAN);
    m_display.draw_hline(0, 30, Display::WIDTH, TFT_GRAY);

    const SpritePixel* f = s_player.current_frame_data();
    if (f != nullptr) {
        int x = (Display::WIDTH - kTestAnim.w) / 2;
        int y = 88 - kTestAnim.h / 2 + 30;
        m_display.draw_sprite(x, y, kTestAnim.w, kTestAnim.h, f);
    }

    char buf[24];
    snprintf(buf, sizeof(buf), "frame %u/%u", s_player.current_frame() + 1, kTestAnim.frames);
