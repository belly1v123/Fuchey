...
// Reusable frame-by-frame animation player for ST7789 240x240.
// Header-only, no heap, no blocking: call tick(now_ms) from the UI
// task at DISPLAY_UPDATE_MS (100ms) and draw() when dirty.
// Supports any number of animations: each SpriteAnim is just
// {w,h,frames,fps,loop,data} generated by scripts/convert_sprite.py.
//
// Limits (documented, not hidden):
// - Frame RAM cost: w*h*2 bytes in flash (const), 0 in PSRAM.
// - SPI cost per frame: w*h*2 bytes via push_window. 96x96=18KB
//   (~20ms @8MHz, 10-12fps comfortable). Full 240x240=115KB (~115ms).
// - Keep per-anim <= 512KB flash (e.g. 96x96x12=216KB). Larger anims
//   belong in SPIFFS with a streaming FrameProvider (future step).
// ============================================================
